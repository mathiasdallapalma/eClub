...
const Eventi = ()=>{
    ...
    const getAllEventi = async(handler) => {
        let response= await Axios.get('https://is-eclub.herokuapp.com/api/v2/event',{
        headers:{
    ...
    const getEventiByTeam = async(handler) => {
        let response= await Axios.get('https://is-eclub.herokuapp.com/api/v2/event/team/'+user.team_id,{
        headers:{
            "auth-token":sessionStorage.getItem('token')}
        })
        handler(response.data);
